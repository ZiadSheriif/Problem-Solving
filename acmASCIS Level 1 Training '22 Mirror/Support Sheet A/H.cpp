...
void solve() {
    ...
    vll temp;
    set<ll> mySet;
    ll mx = vec[0], mn = vec[0], first = vec[0], last = vec[n - 1], mx2 = vec[n - 1], mn2 = vec[n - 1];
    for (int i = 1, j = n - 2; i < n - 1; j--, ++i) {
        // from left to right
        mn = min(mn, vec[i]);
        mx = max(mx, vec[i]);

        if ((vec[i] == mn && vec[i] != first) || (mx != vec[i]))
            mySet.insert(i);
        ...
        if ((vec[j] == mx2 && vec[j] != last) || (mn2 != vec[j]))
            mySet.insert(j);
    }
    cout << (n - 2) - mySet.size() << '\n';
    mySet.clear();
