...
void solve()
{
    ...
    queue<ll> q;
    q.push(n);                     // 4
    step[n] = 0, isVisited[n] = 1; // 4
    while (!q.empty())
    {
        u = q.front(); // 4 get top of queue
        q.pop();       // pop from queue
        for (int i = 1; i <= 2; i++)
        {
            if (i == 1)
                v = u - 1; // 3           //step back one step
            else
                v = u * 2;           // multiply by 2
            if (v < 0 || v >= 10001) // Determine whether to cross the boundary
                continue;
            if (!isVisited[v])
            { // If the conditions are met, join the team and record the number of steps
                q.push(v);
                step[v] = step[u] + 1;
                isVisited[v] = 1;
            }
