...
void solve()
{
    ...
    {
        cin >> l >> r;
        myPair.emplace_back(l, r);
    }
    ll lr = myPair[0].first, rr = myPair[0].second;
    bool test = true;
    for (int i = 1; i < myPair.size(); i++)
    {
        if (myPair[i].first > rr || myPair[i].second < lr)
        {
            test = false;
            break;
        }
        ...
        {
            lr = max(lr, myPair[i].first);
            rr = min(rr, myPair[i].second);
        }
    }
    if (!test)
        cout << 0;
